...
TradingView Webhook → 深币 ETH-USDT-SWAP 永续合约自动化引擎。与币安 VPS 逻辑对齐，单位按 **张** 计算，实盘 **8 倍杠杆**，钉钉为 **紫金主题**。
...
| 端口 | **5004** |
| 杠杆 | **8x**（全仓 cross，开仓前自动 set-leverage） |
| 合约面值 | 0.1 ETH/张 |
| 健康检查 | `GET /health` |
...
5. 开仓量按 `余额 × regime.margin × 8x ÷ (价格 × 0.1)` 取整张数，最低 1 张。
