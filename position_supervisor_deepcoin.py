...
class PositionSupervisor(PipelineBridgeMixin, RadarReentryMixin):
    ...
    def _reconcile_context_on_recover(self, pos):
        ...
        if saved_watched <= 0 and real_amt > 0:
            ...
            if last_open and je > 0 and entry_px > 0 and abs(entry_px - je) > entry_tol:
                ...
                if trusted <= real_amt:
                    # ?????inferred??????? initial > ?????????
                    # ?? TP1/TP2 ??????????????????? tp_levels_consumed
                    # ???"???????"??????? TP1/TP2 ????????????
                    # ??????????? real_amt??????TP ??????????
                    # ?? "reconcile" ?????? initial_qty ?????? TP ???
                    # ???????????? _reconcile_stale_tp_consumed /
                    # _sanitize_tp_consumed ??????
                    self.initial_qty = real_amt
                    notes.append(
    ...
    def _resolve_open_initial_qty(self, live_qty, entry=None):
        """
        ????? initial_qty??????????????????
        ?? bug ??????????? initial_qty ??????? live_qty ????
        ?????TP ??????????????????????? initial_qty ??
        ????? trusted(??journal????) > live_qty(????TP???????)
        ??????????????????????????????? tp_levels_consumed
        ?????????????????"???"?TP1/TP2 ??????????????
        ????????????????????????????? _reconcile_stale_tp_consumed /
        _sanitize_tp_consumed ??????????????
        ...
        if saved > live_qty and trusted <= live_qty:
            ...
            logger.warning(
                f"?? ??? {trusted}? > ?? {live_qty}? ? ???? initial_qty"
                f"?? TP ???????????????????"
            )
            self.initial_qty = live_qty
            self._save_state()
    ...
    def _radar_enforce_regime_cap(self, live_qty, curr_px, force=False):
        ...
        # A regime-cap trim is a deliberate, one-time resize -- the new
        # target becomes the legitimate stable baseline for TP quantity
        # math going forward (_tp_baseline_qty). Previously only
        # initial_qty was updated here, leaving _open_settled_qty stale at
        # the pre-trim size; since _tp_baseline_qty checks
        # _open_settled_qty first, that stale value then got capped down to
        # whatever live_qty happened to be at each subsequent TP-fill check
        # (the same "initial > live_qty -> use live_qty" safety net that's
        # needed for genuine partial fills), reproducing the exact
        # shrinking-percentage chase this baseline fix exists to prevent.
        self._open_settled_qty = new_qty
        if pos:
    ...
    def _tp_baseline_qty(self, fallback=0):
        """
        binance parity (_tp_baseline_qty): ???????????????????
        ??????? live_qty ????????????????????????????
        """
        settled = self._safe_qty(getattr(self, "_open_settled_qty", 0) or 0)
        if settled > 0:
            return settled
        initial = self._safe_qty(getattr(self, "initial_qty", 0) or 0)
        if initial > 0:
            return initial
        return self._safe_qty(fallback or 0)
    ...
    def _split_remaining_tp_quantities(self, live_qty, ratios=None):
        ...
        baseline ??? _open_settled_qty/initial_qty??????????
        ??? live_qty??????????????????? live_qty ???
        """
        live_qty = self._safe_qty(live_qty)
    ...
    def _shield_stop_price(self, entry=None):
        """VPS ??????? _lock_frozen_hard_sl_from_tv ??????1.15x?????
        frozen_hard_sl_px?????????? tv_sl?????????? spec 3.3 ?
        ??|TV.price-TV.stop_loss|x1.15 ?????????? tv_sl(?????)
        ????????????????????????"""
    ...
    def _place_shield_stops(self, live_qty, entry=None, reason="", force=False,
        ...
        # v16.26 ????????????????????? _local_ord ?????????
        # ?? deepcoin ? TPSL ??????ordType ??????'stop'/'trigger'/
        # 'conditional' ????????????? triggerOrderType='TPSL' ???
        # ????????? ordType ??????????? triggerOrderType ??????
        # ??????????????????????????? _shield_sltp_ord_id ??
        # ?????????????????? bug???????????? 601.72 ????
        # 599.42 ??????????????????????????????
        ...
        for t in deepcoin_client.get_trigger_orders_pending(self.symbol):
            ...
            t_trigger_type = str(t.get("triggerOrderType", "")).strip().upper()
            is_shield = (
                (t_trigger_type == "TPSL" or t_ord_type in ("stop", "trigger", "conditional"))
                and t_pos_side == pos_side
            )
