...
DEEPCOIN_SUPERVISOR_VERSION = "v13.15.0-manual-recover-smart"
...
class PositionSupervisor:
    ...
    def _resolve_open_initial_qty(self, live_qty):
        """开单原始张数：state initial_qty 优先，否则开仓日志"""
        ...
        if last_open:
            ...
            if jq > live_qty:
                logger.info(
                    f"📖 开单张数取自开仓日志 {jq} 张 "
                    f"(state initial={saved}, 现仓 {live_qty}张)"
                )
                return jq
    ...
    def _apply_recover_live_alignment(self, side, reconcile):
        """重启以实盘为准：不回放 TV 平仓，不因日志方向差异核武全平（仅 live webhook 才平仓）"""
        extra_notes = []
    ...
    def _process_radar_trailing(self, real_amt, curr_px):
        ...
        if not self._is_radar_active():
            fee_buffer = self.watched_entry * 0.0015
            if self.current_side == "LONG":
                boot_sl = self._clamp_radar_to_tv_floor(
                    max(new_sl, self.watched_entry + fee_buffer)
                )
                if boot_sl > self.current_sl:
                    self.current_sl = boot_sl
            else:
                boot_sl = self._clamp_radar_to_tv_floor(
                    min(new_sl, self.watched_entry - fee_buffer)
                )
            ...
            if pos and self._safe_qty(pos.get("size", 0)) != 0:
                ...
                try:
                    ...
                    if tp_repair.get("repaired") and tp_repair.get("result"):
                        ...
                        sl = self._radar_sl_to_pass()
                        if sl and not self._has_trigger_sl_near(sl):
                            sl_ok = self._ensure_radar_sl(real_amt, sl)
                        ...
                        if saved_sl and radar_active and not self._has_trigger_sl_near(saved_sl):
                            sl_ok = self._ensure_radar_sl(real_amt, saved_sl)
                    ...
                    sl_ok = (
                        not radar_active
                        or self._has_trigger_sl_near(self.current_sl)
                    )
